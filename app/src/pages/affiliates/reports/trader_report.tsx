...
const Page: NextPage = () => {
  ...
  const { data } = api.affiliates.getTraderReport.useQuery({from:selectedDates[0]?.toString(),to:selectedDates[1]?.toString()})
  const {data: merchants} = api.affiliates.getAllMerchants.useQuery();
  const {data: countries} = api.affiliates.getLongCountries.useQuery({table_type:"stats"});
  ...
  return (
    <>
      ...
      />
        ...
        <Select placeholder='Merchant' onChange={(event) => setDisplayType(event.target.value)}>
